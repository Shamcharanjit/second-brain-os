...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Support single invite or batch
    // Single: { waitlist_id: string }
    // Batch:  { batch_size: number }  — auto-selects top priority users
    // Resend: { waitlist_id: string, resend: true }

    const results = { sent: 0, failed: 0, ids: [] as string[] };

    let targets: { id: string; name: string; email: string; invite_token: string | null; invited: boolean; status: string }[] = [];

    if (body.batch_size && typeof body.batch_size === "number") {
      // Batch mode: select top N uninvited pending users by priority
      const { data } = await sb
        .from("waitlist_signups")
        .select("id, name, email, invite_token, invited, status, referral_reward_level, referral_count, created_at")
        .eq("status", "pending")
        .eq("invited", false)
      ...
      const { data } = await sb
        .from("waitlist_signups")
        .select("id, name, email, invite_token, invited, status")
    ...
    for (const user of targets) {
      ...
      const ok = await sendViaSMTP(user.email, subject, buildInviteHtml(user.name, inviteLink), smtpUser, smtpPass);
      ...
      if (ok) {
        ...
        await sb
          ...
          .update({
            ...
            invite_token: token,
            invite_sent_at: now,
            last_email_type_sent: isResend ? "invite_resend" : "invite",
            email_send_count: (user as any).email_send_count ? (user as any).email_send_count + 1 : 1,
          })
          .eq("id", user.id);
    ...
    console.log("Invite emails sent:", results);
