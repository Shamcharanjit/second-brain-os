...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'clicks', total_clicks_kw,
    'ctr', CASE WHEN total_impr > 0 THEN ROUND((total_clicks_kw::numeric / total_impr) * 100, 2) ELSE 0 END
  ) ORDER BY total_impr DESC), '[]'::jsonb)
  INTO v_top_keywords
  FROM (
    SELECT kw,
           SUM(impressions)::int AS total_impr,
           SUM(clicks)::int AS total_clicks_kw
    FROM public.seo_metadata,
         LATERAL unnest(COALESCE(keywords, ARRAY[]::text[])) AS kw
    GROUP BY kw
  ) keyword_stats;
  ...
  RETURN jsonb_build_object(
    ...
    'overall_ctr', CASE WHEN v_total_impressions > 0 THEN ROUND((v_total_clicks::numeric / v_total_impressions) * 100, 2) ELSE 0 END
  );
END;
$function$;