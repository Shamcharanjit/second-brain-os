        ...
        <div className="space-y-6 pt-2">
            ...
            <div className="space-y-1.5">
              {activeActions.map((a) => (
                <div key={a.id} className={`rounded-lg border p-3 flex items-center gap-2 ${a.is_primary ? "bg-primary/5 border-primary/15" : "bg-card"}`}>
                  <button onClick={() => { completeNextAction(projectId, a.id); toast.success("Completed"); }}>
          ...
          <div className="space-y-2">
            ...
            {linkedTasks.length === 0 ? (
              ...
              <div className="space-y-2">{linkedTasks.slice(0, 5).map((c) => <CaptureCard key={c.id} capture={c} />)}</div>
            )}
          </div>
          ...
          <div className="space-y-2">
            ...
            {sourceIdea && (
              <div className="rounded-lg bg-[hsl(var(--brain-purple))/0.08] border border-[hsl(var(--brain-purple))/0.15] p-2.5 flex items-center gap-2 text-xs">
                ...
                <span className="text-muted-foreground">Originated from: <span className="font-medium text-foreground">{sourceIdea.ai_data?.title}</span></span>
              </div>
            )}
            {linkedIdeas.length === 0 && !sourceIdea ? (
              <p className="text-xs text-muted-foreground py-2">No ideas linked yet.</p>
            ) : (
              <div className="space-y-2">{linkedIdeas.slice(0, 3).map((c) => <CaptureCard key={c.id} capture={c} />)}</div>
            )}
          </div>
