...
import {
  LogIn, UserPlus, Loader2, Shield, Smartphone, Cloud,
  ArrowLeft, Brain, Zap, Search,
} from "lucide-react";
...
const BENEFITS = [
  ...
  { icon: Shield,     text: "End-to-end encrypted backups" },
  { icon: Zap,        text: "AI captures and triages everything" },
  { icon: Search,     text: "Semantic memory search" },
...
export default function AuthPage() {
  ...
  /* Force dark mode to match the rest of the public pages */
  useEffect(() => {
    ...
    <div className="min-h-screen flex flex-col bg-background text-foreground">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
        <div className="mx-auto max-w-6xl px-5 md:px-8 flex items-center justify-between h-14">
          ...
          <Button variant="ghost" size="sm" onClick={() => navigate("/waitlist")} className="text-sm hover:bg-muted">
            Join Waitlist
          </Button>
        </div>
      </header>
      ...
      {/* ── Main ── */}
      <div className="flex-1 flex items-center justify-center pt-14 px-4 py-12">
        <div className="w-full max-w-sm space-y-6">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              {mode === "login"
                ...
                : "Start capturing, organising and recalling with AI"
              }
            </p>
          </div>
          ...
          {blocked && (
            <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 space-y-3 text-center">
              <p className="text-sm font-medium text-foreground">
                InsightHalo is in early access
              </p>
              <p className="text-xs text-muted-foreground">
                This email hasn't been invited yet. Join the waitlist — we're opening access in batches.
              </p>
              <Button size="sm" onClick={() => navigate("/waitlist")} className="gap-1.5 w-full">
                Join the Waitlist <ArrowLeft className="h-3.5 w-3.5 rotate-180" />
              </Button>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <Input
              ...
              className={`h-11 ${inviteLocked ? "bg-muted cursor-not-allowed" : ""}`}
            />
            <Input
            ...
            <div className="rounded-xl border border-border/60 bg-card/50 p-4 space-y-2.5">
              ...
              {BENEFITS.map((b) => (
                <div key={b.text} className="flex items-center gap-2.5">
                  <b.icon className="h-3.5 w-3.5 text-primary/70 shrink-0" />
        ...
        <div className="mx-auto max-w-6xl px-5 md:px-8 py-5">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-5 text-xs text-muted-foreground">
              ...
              <button onClick={() => navigate("/waitlist")} className="hover:text-foreground transition-colors">Waitlist</button>
